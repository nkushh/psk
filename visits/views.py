...
def coordinators_aggregate_visits(request):
	...
	visits = Visit.objects.values('supervisor').annotate(total_visits=Count('id')).order_by('-total_visits')
...
def record_visit(request):
	...
	if request.method == "POST":
			supervisor = account
			if Facilities.objects.filter(facility_name=request.POST['facility']).exists():
				try:
				...
				messages.error(request, "Error! Facility {} details do not exist in our master list. Kindly add them.".format(request.POST['facility']))
				return redirect("facilities:new_facility")

			if Visit.objects.filter(Q(facility=facility, visit_date=visit_date) | Q(facility=facility, ld_invoice_no=ld_invoice_no) | Q(facility=facility, lld_invoice_no=lld_invoice_no)).exists():
